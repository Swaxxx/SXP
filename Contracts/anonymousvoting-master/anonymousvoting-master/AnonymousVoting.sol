...
contract AnonymousVoting is owned {
  ...
  function getVoter() public returns (uint[2] _registeredkey, uint[2] _reconstructedkey, bytes32 _commitment) {
  ...
  // to complete the job...
  function AnonymousVoting(uint _gap, address _charity) public payable  {
    G[0] = Gx;
    G[1] = Gy;
    state = State.SETUP;
  ...
  function beginSignUp(string _question, bool enableCommitmentPhase, uint _finishSignupPhase, uint _endSignupPhase, uint _endCommitmentPhase, uint _endVotingPhase, uint _endRefundPhase, uint _depositrequired) inState(State.SETUP) public onlyOwner payable returns (bool){
  ...
  function register(uint[2] xG, uint[3] vG, uint r) inState(State.SIGNUP) public payable returns (bool) {

     // HARD DEADLINE
	 // returns the voter's ether, but exhausts their gas.
	 assert(block.timestamp <= finishSignupPhase);
  ...
  function submitVote(uint[4] params, uint[2] y, uint[2] a1, uint[2] b1, uint[2] a2, uint[2] b2) public inState(State.VOTE) returns (bool) {
  ...
  function verify1outof2ZKP(uint[4] params, uint[2] y, uint[2] a1, uint[2] b1, uint[2] a2, uint[2] b2) public returns (bool) {
      uint[2] memory temp1;
      uint[3] memory temp2;
      uint[3] memory temp3;
